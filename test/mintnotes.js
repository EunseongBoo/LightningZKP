...
contract('mintNote', function(accounts) {
  ...
  beforeEach(async () => {
    zkdai = await ZkDai.new(100, 1e18.toString(), dai.address);
  })

  it('transfers dai and mints note', async function() {
    // check dai balance and approve the zkdai contract to be able to move tokens
    //await dai.approve(zkdai.address, web3.utils.toBN(100 * SCALING_FACTOR));
    assert.equal(await dai.balanceOf.call(accounts[0]), 100*SCALING_FACTOR , '100 dai tokens were not assigned to the 1st accounct' + await dai.balanceOf.call(accounts[0]));
    assert.equal(await dai.balanceOf.call(zkdai.address), 0, 'Zkdai contract should have 0 dai tokens');
    await dai.approve(zkdai.address, web3.utils.toBN(5 * SCALING_FACTOR));
