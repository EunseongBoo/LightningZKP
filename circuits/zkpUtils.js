/**
@module utils.js
@author Westlad,Chaitanya-Konda,iAmMichaelConnor from nightfall project //(https://github.com/EYBlockchain/nightfall)
...
function getLeafIndexFromZCount_miniMerkle(zCount) {
  // force it to be a number:
  const zCountInt = parseInt(zCount, 10);
  const MERKLE_DEPTH = parseInt(merkleDepth_lzkp, 10);
  const MERKLE_WIDTH = parseInt(2 ** (MINI_MERKLE_DEPTH - 1), 10);
  const leafIndex = parseInt(MERKLE_WIDTH - 1 + zCountInt, 10);
  return leafIndex;
}
