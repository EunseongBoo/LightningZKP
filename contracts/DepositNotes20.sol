...
import "./verifiers/DepositNote20Verifier.sol";
import "./ZkDaiBase.sol";
import "./DepositBase.sol";

contract DepositNotes20 is DepositNote20Verifier, ZkDaiBase, DepositBase{
  uint8 internal constant NUM_PUBLIC_INPUTS = 91;
  uint8 internal constant NUM_DEPOSIT_NOTES = 20*2; //deposit_note_num * 2 (sender and receiver)
  ...
  */
  function depositCommit_20(bytes32 proofHash)
    internal
  {
      Submission storage submission = submissions[proofHash];
      bytes32[NUM_ALL_NOTES] memory _notes = getALLNotes_20(submission.publicInput);
  ...
  function getALLNotes_20(uint256[] memory input)
    internal
    pure
  ...
  */
    ...
    internal
  {
      ...
      if (!DepositNote20Verifier.depositVerifyTx(a, b, c, input)) {
        // challenge passed
        delete submissions[proofHash];
